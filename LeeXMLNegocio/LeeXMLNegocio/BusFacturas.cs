...
namespace LeeXMLNegocio
{
	public class BusFacturas : BusAbstracta
	{
		...
		public List<EntFactura> ObtieneComprobantesFiscales(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					p.IVA = Convert.ToDecimal(r["FAC_IVA"]);
					p.IEPS = Convert.ToDecimal(r["FAC_IEPS"]);
					p.ImpuestosLocales = Convert.ToDecimal(r["FAC_IMPUESTOLOCAL"]);
					p.Total = Convert.ToDecimal(r["FAC_TOTAL"]);
					p.TotalUSD = Convert.ToDecimal(r["FAC_TOTALUSD"]);
					p.TotalPercepciones = Convert.ToDecimal(r["FAC_TOTALPERCEPCIONES"]);
					p.TotalDeducciones = Convert.ToDecimal(r["FAC_TOTALDEDUCCIONES"]);
					p.SubTotal = p.Total - p.IVA + p.Retenciones - p.IEPS + p.ImpuestosLocales;
					p.Descuento = Convert.ToDecimal(r["FAC_SUBTOTALTASA0"]);
					p.ImporteDR = p.SubTotal + p.Descuento;
					p.Fecha = Convert.ToDateTime(r["FAC_FECHA"]);
					p.MonedaId = Convert.ToInt32(r["FAC_MONEDAID"]);
					p.Moneda = r["CATMON_CLAVE"].ToString();
					p.TipoCambio = Convert.ToDecimal(r["FAC_TIPOCAMBIO"]);
					p.MetodoPagoId = Convert.ToInt32(r["FAC_METODOPAGOID"]);
					p.MetodoPago = r["CATMET_CLAVE"].ToString();
					p.FormaPagoId = Convert.ToInt32(r["FAC_FORMAPAGOID"]);
					p.FormaPago = r["CATFOR_DESCRIPCION"].ToString();
					p.UUID = r["FAC_UUID"].ToString();
					p.Descripcion = r["FAC_CONCEPTO"].ToString();
					p.UsoCFDI = r["CATUSO_CLAVE"].ToString() + " - " + r["CATUSO_DESCRIPCION"].ToString();
					p.FacturaRelacionId = Convert.ToInt32(r["FAC_FACTURARELACIONADAID"]);
					p.UUIDrelacionado = r["FAC_UUIDRELACIONADO"].ToString();
					p.TipoRelacionId = Convert.ToInt32(r["FAC_TIPORELACIONFACTURAID"]);
					p.FechaPago = Convert.ToDateTime(r["FAC_FECHAPAGO"]);
		...
		public List<EntFactura> ObtieneComprobantesFiscalesEgresos(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				...
				foreach (DataRow r in dt.Rows)
				{
					EntFactura p = new EntFactura();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
					p.VersionCFDIId = Convert.ToInt32(r["FAC_VERSIONCFDIID"]);
					p.Nombre = Empresa.NombreFiscal;
					p.RFC = Empresa.RFC;
		...
		public List<EntFactura> ObtieneComprobantesFiscales(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					p.TotalPercepciones = Convert.ToDecimal(r["FAC_TOTALPERCEPCIONES"]);
					p.TotalDeducciones = Convert.ToDecimal(r["FAC_TOTALDEDUCCIONES"]);
					p.SubTotal = p.Total - p.IVA - p.IEPS + p.Retenciones + p.ImpuestosLocales;
					p.Descuento = Convert.ToDecimal(r["FAC_SUBTOTALTASA0"]);
		...
		public List<EntFactura> ObtieneComprobantesFiscalesObjetoImpuesto(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				List<EntFactura> lst = new List<EntFactura>();
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					p.SubTotal = p.Total - p.IVA + p.Retenciones - p.IEPS;
					p.Descuento = Convert.ToDecimal(r["FAC_SUBTOTALTASA0"]);
					if (p.VersionCFDIId == 2)
					{
						p.ObjetoImpuestoId = Convert.ToInt32(r["FAC_OBJETOIMPUESTOID"]);
						p.SubTotal = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO02"]);
						p.Total = p.SubTotal + p.IVA - p.Retenciones + p.IEPS;
						if (p.TipoComprobanteId == 2)
						{
							p.ObjetoImpuesto01 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO01"]) * -1m;
							p.ObjetoImpuesto02 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO02"]) * -1m;
							p.ObjetoImpuesto03 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO03"]) * -1m;
						}
						else
						{
							p.ObjetoImpuesto01 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO01"]);
							p.ObjetoImpuesto02 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO02"]);
							p.ObjetoImpuesto03 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO03"]);
						}
					}
					p.Pago = Convert.ToDecimal(r["FAC_PAGO"]);
					p.Deposito = Convert.ToDecimal(r["FAC_DEPOSITO"]);
		...
		public List<EntFactura> ObtieneComprobantesFiscalesPorFechaPagoExclusivoObjetoImpuesto(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				List<EntFactura> lst = new List<EntFactura>();
		...
		public List<EntFactura> ObtieneComprobantesFiscalesDatosAdicionalesTraslado(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				List<EntFactura> lst = new List<EntFactura>();
				dt = new DatFacturas().obtieneComprobantesFiscalesDatosAdicionalesTraslado(Empresa.Id, FechaDesde, FechaHasta, EstatusId);
				foreach (DataRow r in dt.Rows)
				{
					EntFactura p = new EntFactura();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
					...
					p.SubTotal = p.Total - p.IVA + p.Retenciones - p.IEPS;
					p.Descuento = Convert.ToDecimal(r["FAC_SUBTOTALTASA0"]);
					p.Total = Convert.ToDecimal(r["FACDAT_BASEDR"]);
					p.ImporteDR = Convert.ToDecimal(r["FACDAT_IMPORTEDR"]);
					p.TipoFactorId = Convert.ToInt32(r["FACDAT_TIPOFACTORDRID"]);
					p.TasaOCuota = Convert.ToDecimal(r["FACDAT_TASAOCUOTADR"]);
					p.ObjetoImpuestoId = Convert.ToInt32(r["FAC_OBJETOIMPUESTOID"]);
					p.ObjetoImpuesto01 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO01"]);
					p.ObjetoImpuesto02 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO02"]);
					p.ObjetoImpuesto03 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO03"]);
		...
		public List<EntFactura> ObtieneComprobantesFiscalesPorFechaPagoExclusivoDatosAdicionalesTraslado(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				List<EntFactura> lst = new List<EntFactura>();
				dt = new DatFacturas().obtieneComprobantesFiscalesPorFechaPagoExclusivoDatosAdicionalesTraslado(Empresa.Id, FechaDesde, FechaHasta, EstatusId);
				foreach (DataRow r in dt.Rows)
				{
					EntFactura p = new EntFactura();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
		...
		public List<EntFactura> ObtieneComprobantesFiscalesEgresos(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					p.Nombre = r["PROV_NOMBREFISCAl"].ToString();
					p.RFC = r["PROV_RFC"].ToString();
					p.EmisorNombre = Empresa.NombreFiscal;
					p.EmisorRFC = Empresa.RFC;
					p.TipoComprobanteId = Convert.ToInt32(r["FAC_TIPOCOMPROBANTEID"]);
					p.TipoComprobante = r["CATCOM_DESCRIPCION"].ToString();
					p.SerieFactura = r["FAC_SERIEFACTURA"].ToString();
					...
					p.Descripcion = r["FAC_CONCEPTO"].ToString();
					p.UsoCFDI = r["CATUSO_CLAVE"].ToString() + " - " + r["CATUSO_DESCRIPCION"].ToString();
					p.UsoCFDIId = Convert.ToInt32(r["FAC_USOCFDIID"]);
					p.FacturaRelacionId = Convert.ToInt32(r["FAC_FACTURARELACIONADAID"]);
					p.UUIDrelacionado = r["FAC_UUIDRELACIONADO"].ToString();
					p.TipoRelacionId = Convert.ToInt32(r["FAC_TIPORELACIONFACTURAID"]);
					p.FechaPago = Convert.ToDateTime(r["FAC_FECHAPAGO"]);
		...
		public List<EntFactura> ObtieneComprobantesFiscalesEgresosObjetoImpuesto(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				List<EntFactura> lst = new List<EntFactura>();
		...
		public List<EntFactura> ObtieneComprobantesFiscalesEgresosPorFechaPagoExclusivoObjetoImpuesto(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				List<EntFactura> lst = new List<EntFactura>();
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					if (p.VersionCFDIId == 2)
					{
						...
						p.SubTotal = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO02"]);
						p.Total = p.SubTotal + p.IVA - p.Retenciones + p.IEPS;
						if (p.TipoComprobanteId == 2 || !p.Deducible)
						{
							p.ObjetoImpuesto01 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO01"]) * -1m;
							p.ObjetoImpuesto02 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO02"]) * -1m;
							p.ObjetoImpuesto03 = Convert.ToDecimal(r["FAC_OBJETOIMPUESTO03"]) * -1m;
		...
		public List<EntFactura> ObtieneComprobantesFiscalesEgresosDatosAdicionalesTraslado(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				List<EntFactura> lst = new List<EntFactura>();
				dt = new DatFacturas().obtieneComprobantesFiscalesEgresosDatosAdicionalesTraslado(Empresa.Id, FechaDesde, FechaHasta, EstatusId);
				foreach (DataRow r in dt.Rows)
				{
					EntFactura p = new EntFactura();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
					...
					if (p.TipoComprobanteId == 2)
					{
						p.Total = Convert.ToDecimal(r["FACDAT_BASEDR"]) * -1m;
						p.ImporteDR = Convert.ToDecimal(r["FACDAT_IMPORTEDR"]) * -1m;
					}
					else
					{
						p.Total = Convert.ToDecimal(r["FACDAT_BASEDR"]);
		...
		public List<EntFactura> ObtieneComprobantesFiscalesEgresosPorFechaPagoExclusivoDatosAdicionalesTraslado(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int EstatusId)
		{
			try
			{
				List<EntFactura> lst = new List<EntFactura>();
				dt = new DatFacturas().obtieneComprobantesFiscalesEgresosPorFechaPagoExclusivoDatosAdicionalesTraslado(Empresa.Id, FechaDesde, FechaHasta, EstatusId);
				foreach (DataRow r in dt.Rows)
				{
					EntFactura p = new EntFactura();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
		...
		public List<EntCatalogoPercepciones> ObtieneComprobantesFiscalesNominasPercepcionesXML(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				List<EntCatalogoPercepciones> lst = new List<EntCatalogoPercepciones>();
				dt = new DatFacturas().obtieneComprobantesFiscalesNominasPercepciones(Empresa.Id, FechaDesde, FechaHasta);
				foreach (DataRow r in dt.Rows)
				{
					EntCatalogoPercepciones p = new EntCatalogoPercepciones();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
					...
					p.TipoRegimen = r["FAC_TIPOREGIMEN"].ToString();
					p.Clave = r["FACPER_TIPOPERCEPCION"].ToString();
					p.Descripcion = r["CATPER_DESCRIPCION"].ToString();
					p.Excento = Convert.ToDecimal(r["FACPER_IMPORTEEXENTO"]);
					p.Gravado = Convert.ToDecimal(r["FACPER_IMPORTEGRAVADO"]);
					lst.Add(p);
				}
		...
		public List<EntCatalogoPercepciones> ObtieneComprobantesFiscalesNominasPercepcionesPorFechaFinXML(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				List<EntCatalogoPercepciones> lst = new List<EntCatalogoPercepciones>();
				dt = new DatFacturas().obtieneComprobantesFiscalesNominasPercepcionesPorFechaFin(Empresa.Id, FechaDesde, FechaHasta);
				foreach (DataRow r in dt.Rows)
				{
					EntCatalogoPercepciones p = new EntCatalogoPercepciones();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
					...
					p.Fecha = Convert.ToDateTime(r["FAC_FECHA"]);
					p.FechaPago = Convert.ToDateTime(r["FAC_FECHAPAGO"]);
					p.FechaInicio = Convert.ToDateTime(r["FAC_FECHAINICIO"]);
					p.FechaFin = Convert.ToDateTime(r["FAC_FECHAFIN"]);
					p.ClaveEntFed = r["FAC_CLAVEENTFED"].ToString();
					p.TipoRegimen = r["FAC_TIPOREGIMEN"].ToString();
					p.Clave = r["FACPER_TIPOPERCEPCION"].ToString();
					p.Descripcion = r["CATPER_DESCRIPCION"].ToString();
		...
		public List<EntCatalogoPercepciones> ObtieneComprobantesFiscalesNominasPercepcionesAuxiliar(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				List<EntCatalogoPercepciones> lst = new List<EntCatalogoPercepciones>();
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					p.Fecha = Convert.ToDateTime(r["FAC_FECHA"]);
					p.FechaPago = Convert.ToDateTime(r["FAC_FECHAPAGO"]);
					p.FechaCorta = p.Fecha.ToShortDateString();
					p.TipoRegimen = r["FAC_UUID"].ToString();
					p.ClaveEntFed = r["FAC_CLAVEENTFED"].ToString();
					p.Clave = r["FACPER_TIPOPERCEPCION"].ToString();
					p.Descripcion = r["CATPER_DESCRIPCION"].ToString();
					p.EstatusDescripcion = r["FACPER_CONCEPTO"].ToString();
					p.Excento = Convert.ToDecimal(r["FACPER_IMPORTEEXENTO"]);
					p.Gravado = Convert.ToDecimal(r["FACPER_IMPORTEGRAVADO"]);
					lst.Add(p);
				}
		...
		public List<EntCatalogoPercepciones> ObtieneComprobantesFiscalesNominasPercepcionesAuxiliarPorFechaFin(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				List<EntCatalogoPercepciones> lst = new List<EntCatalogoPercepciones>();
		...
		public List<EntCatalogoPercepciones> ObtieneComprobantesFiscalesNominasDeduccionesXML(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				List<EntCatalogoPercepciones> lst = new List<EntCatalogoPercepciones>();
				dt = new DatFacturas().obtieneComprobantesFiscalesNominasDeducciones(Empresa.Id, FechaDesde, FechaHasta);
				foreach (DataRow r in dt.Rows)
				{
					EntCatalogoPercepciones p = new EntCatalogoPercepciones();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
					...
					p.ClaveEntFed = r["FAC_CLAVEENTFED"].ToString();
					p.TipoRegimen = r["FAC_TIPOREGIMEN"].ToString();
					p.Clave = r["FACDED_TIPODEDUCCION"].ToString();
					p.Descripcion = r["CATDED_DESCRIPCION"].ToString();
					p.Excento = Convert.ToDecimal(r["FACDED_IMPORTEEXENTO"]);
					p.Gravado = Convert.ToDecimal(r["FACDED_IMPORTEGRAVADO"]);
					lst.Add(p);
				}
				return lst;
			}
		...
		public List<EntCatalogoPercepciones> ObtieneComprobantesFiscalesNominasDeduccionesPorFechaFinXML(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				List<EntCatalogoPercepciones> lst = new List<EntCatalogoPercepciones>();
				dt = new DatFacturas().obtieneComprobantesFiscalesNominasDeduccionesPorFechaFin(Empresa.Id, FechaDesde, FechaHasta);
				foreach (DataRow r in dt.Rows)
				{
					EntCatalogoPercepciones p = new EntCatalogoPercepciones();
					p.Id = Convert.ToInt32(r["FAC_ID"]);
		...
		public List<EntCatalogoPercepciones> ObtieneComprobantesFiscalesNominasDeduccionesAuxiliar(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				List<EntCatalogoPercepciones> lst = new List<EntCatalogoPercepciones>();
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					p.Clave = r["FACDED_TIPODEDUCCION"].ToString();
					p.Descripcion = r["CATDED_DESCRIPCION"].ToString();
					p.EstatusDescripcion = r["FACDED_CONCEPTO"].ToString();
					p.Excento = Convert.ToDecimal(r["FACDED_IMPORTEEXENTO"]);
					p.Gravado = Convert.ToDecimal(r["FACDED_IMPORTEGRAVADO"]);
					lst.Add(p);
				}
		...
		public List<EntCatalogoPercepciones> ObtieneComprobantesFiscalesNominasDeduccionesAuxiliarPorFechaFin(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			try
			{
				List<EntCatalogoPercepciones> lst = new List<EntCatalogoPercepciones>();
		...
		public List<EntFactura> ObtieneSaldoAFecha(EntEmpresa Empresa, DateTime FechaHasta, int ClienteDesdeId, int ClienteHastaId)
		{
			try
			{
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					p.MonedaId = Convert.ToInt32(r["MONEDAID"]);
		...
		public List<EntEstadoDeCuenta> ObtieneCxCPorClientes(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int ClienteDesdeId, int ClienteHastaId)
		{
			try
			{
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					p.MonedaId = Convert.ToInt32(r["MONEDAID"]);
					p.Moneda = r["MONEDA"].ToString();
					if (lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p.ClienteId && P.MonedaId == p.MonedaId).ToList().Count == 0)
					{
						p.SubTotal0 = 0m;
					}
					else
					{
						p.SubTotal0 = lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p.ClienteId && P.MonedaId == p.MonedaId).ToList()[0].Descuento;
					}
		...
		public List<EntEstadoDeCuenta> ObtieneCxCMovimientosPorClientes(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta, int ClienteDesdeId, int ClienteHastaId)
		{
			try
			{
				...
				decimal saldoUSD = default(decimal);
				decimal saldoFinalUSD = default(decimal);
				int ordenId = 0;
				...
				foreach (DataRow r in dt.Rows)
				{
					...
					if (p.MonedaId == 1)
					{
						if (clienteId != p.ClienteId)
						{
							...
							{
								saldo = lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p.ClienteId && P.MonedaId == p.MonedaId).ToList()[0].Descuento;
								saldoFinal += saldo;
							}
							p.SubTotal0 = saldo;
						}
					...
					else
					{
						if (clienteId != p.ClienteId)
						{
							...
							if (lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p.ClienteId && P.MonedaId == p.MonedaId).ToList().Count == 0)
							{
								saldoUSD = default(decimal);
							}
							else
							{
								saldoUSD = lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p.ClienteId && P.MonedaId == p.MonedaId).ToList()[0].Descuento;
								saldoFinalUSD += saldoUSD;
							}
							p.SubTotal0 = saldoUSD;
						}
						if (p.TipoComprobanteId == 1)
						{
							p.SubTotal = Convert.ToDecimal(r["FAC_TOTAL"]);
							saldoUSD += p.SubTotal;
							saldoFinalUSD += p.SubTotal;
						}
						else
						{
							p.Total = Convert.ToDecimal(r["FAC_TOTAL"]);
							saldoUSD -= p.Total;
							saldoFinalUSD -= p.Total;
						}
						p.Descuento = saldoUSD;
						p.Deposito = saldoFinalUSD;
					}
					lst.Add(p);
				}
				foreach (EntFactura f in lstSaldosIniciales)
				{
					if (lst.Where((EntEstadoDeCuenta P) => P.ClienteId == f.ClienteId && P.MonedaId == 1).ToList().Count == 0 && f.MonedaId == 1 && f.Descuento > 0m)
					{
					...
					else if (lst.Where((EntEstadoDeCuenta P) => P.ClienteId == f.ClienteId && P.MonedaId == 2).ToList().Count == 0 && f.MonedaId == 2 && f.Descuento > 0m)
					{
						saldoFinalUSD += f.Descuento;
						lst.Add(new EntEstadoDeCuenta
						{
							ClienteId = f.ClienteId,
							Nombre = f.Nombre,
			...
			try
			{
					...
					select P).ToList())
				{
					if (clienteId != p2.ClienteId)
					{
						...
						if (lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p2.ClienteId && p2.MonedaId == 1).ToList().Count == 0)
						{
							saldo = default(decimal);
						}
						else
						{
							saldo = lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p2.ClienteId && p2.MonedaId == 1).ToList()[0].Descuento;
							saldoFinal += saldo;
						}
						p2.SubTotal0 = saldo;
					}
				...
				foreach (EntEstadoDeCuenta p3 in (from P in lst
					where P.MonedaId == 2
					orderby P.ClienteId, P.UUIDrelacionado, P.Fecha
					select P).ToList())
				{
					if (clienteId != p3.ClienteId)
					{
						clienteId = p3.ClienteId;
						if (lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p3.ClienteId && p3.MonedaId == 2).ToList().Count == 0)
						{
							saldoUSD = default(decimal);
						}
						else
						{
							saldoUSD = lstSaldosIniciales.Where((EntFactura P) => P.ClienteId == p3.ClienteId && p3.MonedaId == 2).ToList()[0].Descuento;
							saldoFinalUSD += saldoUSD;
						}
						p3.SubTotal0 = saldoUSD;
					}
					if (p3.TipoComprobanteId == 1)
					{
						saldoUSD += p3.SubTotal;
						saldoFinalUSD += p3.SubTotal;
					}
					else
					{
						saldoUSD -= p3.Total;
						saldoFinalUSD -= p3.Total;
					}
					p3.Descuento = saldoUSD;
					p3.Deposito = saldoFinalUSD;
				}
				foreach (EntFactura f in lstSaldosIniciales)
				{
					if (lst.Where((EntEstadoDeCuenta P) => P.ClienteId == f.ClienteId && P.MonedaId == 1).ToList().Count == 0 && f.MonedaId == 1 && f.Descuento > 0m)
		...
		public int AgregaComprobantesFiscales(EntFactura Factura)
		{
			try
			{
				return new DatFacturas().agregaComprobanteFiscal(Factura.EmpresaId, Factura.TipoComprobanteId, Factura.ClienteId, Factura.SerieFactura, Factura.NumeroFactura, Factura.Fecha, Factura.IVARetenciones, Factura.ISRRetenciones, Factura.IVA, Factura.SubTotal, Factura.SubTotal0, Factura.Total, Factura.MonedaId, Factura.FormaPagoId, Factura.MetodoPagoId, Factura.UUID, Factura.Descripcion, Factura.FacturaRelacionId, Factura.UUIDrelacionado, Factura.TipoRelacionId, Factura.FechaPago, Factura.Ruta, Factura.UsuarioId);
			}
			catch (Exception ex)
			{
				throw new Exception(ex.Message);
		...
		public int AgregaComprobantesFiscalesNeue(EntFactura Factura)
		{
			try
			{
				return new DatFacturas().agregaComprobanteFiscalNeue(Factura.EmpresaId, Factura.TipoComprobanteId, Factura.ClienteId, Factura.SerieFactura, Factura.NumeroFactura, Factura.Fecha, Factura.IVARetenciones, Factura.ISRRetenciones, Factura.IVA, Factura.IEPS, Factura.ImpuestosLocales, Factura.SubTotal, Factura.Descuento, Factura.TotalPercepciones, Factura.TotalDeducciones, Factura.Total, Factura.TotalUSD, Factura.MonedaId, Factura.TipoCambio, Factura.ObjetoImpuestoId, Factura.ObjetoImpuesto01, Factura.ObjetoImpuesto02, Factura.ObjetoImpuesto03, Factura.ObjetoImpuesto04, Factura.FormaPagoId, Factura.MetodoPagoId, Factura.UUID, Factura.Descripcion, Factura.FacturaRelacionId, Factura.UUIDrelacionado, Factura.TipoRelacionId, Factura.FechaPago, Factura.FechaInicio, Factura.FechaFin, Factura.ClaveEntFed, Factura.TipoRegimen, Factura.UsoCFDI, Factura.Ruta, Factura.VersionCFDIId, Factura.UsuarioId);
			}
			catch (Exception ex)
			{
				throw new Exception(ex.Message);
		...
		public int AgregaComprobantesFiscalesDatosAdicionales(EntFactura Factura)
		{
			try
			{
				return new DatFacturas().agregaComprobanteFiscalDatosAdicionales(Factura.Id, Factura.SerieFactura + Factura.NumeroFactura, Factura.Fecha, Factura.TipoEstructuraImpuestoId, Factura.ObjetoImpuestoId, Factura.Total, Factura.TasaOCuota, Factura.TipoFactorId, Factura.TipoImpuestoId, Factura.ImporteDR);
			}
			catch (Exception ex)
			{
				throw new Exception(ex.Message);
		...
		public int AgregaComprobantesFiscalesEgresoNeue(EntFactura Factura)
		{
			try
			{
				return new DatFacturas().agregaComprobanteFiscalEgresoNeue(Factura.EmpresaId, Factura.TipoComprobanteId, Factura.ClienteId, Factura.SerieFactura, Factura.NumeroFactura, Factura.Fecha, Factura.IVARetenciones, Factura.ISRRetenciones, Factura.IVA, Factura.IEPS, Factura.SubTotal, Factura.Descuento, Factura.Total, Factura.TotalUSD, Factura.MonedaId, Factura.TipoCambio, Factura.ObjetoImpuestoId, Factura.ObjetoImpuesto01, Factura.ObjetoImpuesto02, Factura.ObjetoImpuesto03, Factura.FormaPagoId, Factura.MetodoPagoId, Factura.UUID, Factura.Descripcion, Factura.FacturaRelacionId, Factura.UUIDrelacionado, Factura.TipoRelacionId, Factura.FechaPago, Factura.UsoCFDI, Factura.Ruta, Factura.VersionCFDIId, Factura.UsuarioId);
			}
			catch (Exception ex)
			{
				throw new Exception(ex.Message);
		...
		public int AgregaComprobantesFiscalesEgresosDatosAdicionales(EntFactura Factura)
		{
			try
			{
				return new DatFacturas().agregaComprobanteFiscalEgresosDatosAdicionales(Factura.Id, Factura.SerieFactura + Factura.NumeroFactura, Factura.Fecha, Factura.TipoEstructuraImpuestoId, Factura.ObjetoImpuestoId, Factura.Total, Factura.TasaOCuota, Factura.TipoFactorId, Factura.TipoImpuestoId, Factura.ImporteDR);
			}
			catch (Exception ex)
			{
				throw new Exception(ex.Message);
