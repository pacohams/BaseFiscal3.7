...
{
	public class EstadoDeCuentaEgresos : FormBase
	{
		...
		private System.Windows.Forms.Button btnOtraFormaPago;
		...
		public List<EntEstadoDeCuenta> ExcelToListaPedidos(string PathExcel)
		{
			...
			List<EntEstadoDeCuenta> resultado = (from row in (IQueryable<Row>)book.Worksheet(0)
				...
				{
					Total = ConvierteTextoADecimal((string)row["RETIRO"]),
		...
		public void CargaCFDIsinRelacionarEgresos(EntEmpresa Empresa, DateTime FechaDesde, DateTime FechaHasta)
		{
			gvCFDIsSinRelacionar.DataSource = new BusFacturas().ObtieneComprobantesFiscalesSinRelacionarEgresos(Empresa, FechaDesde, FechaHasta);
			gvCFDIsSinRelacionar.Refresh();
		}

		public void CargaEstadoDeCuentasImportadosEgresos(int BancoId, DateTime FechaDesde, DateTime FechaHasta)
		{
			if (BancoId <= 0)
			{
				gvEstadoDeCuentasImportados.DataSource = new BusFacturas().ObtieneEstadosDeCuentaConDetalleEgresos(Program.EmpresaSeleccionada, FechaDesde, FechaHasta);
			}
			else
			{
				gvEstadoDeCuentasImportados.DataSource = (from P in new BusFacturas().ObtieneEstadosDeCuentaConDetalleEgresos(Program.EmpresaSeleccionada, FechaDesde, FechaHasta)
		...
		private void AgregarEstadoDeCuentaEgresos(EntEstadoDeCuenta EstadoDeCuenta)
		{
			EstadoDeCuenta.Id = new BusFacturas().AgregaEstadoDeCuentaEgresos(EstadoDeCuenta);
			foreach (EntFactura f in EstadoDeCuenta.ListaComprobantes)
			{
				if (f.Id == -1)
				{
					f.Id = new BusFacturas().AgregaComprobantesFiscalesEgreso(f);
				}
				new BusFacturas().AgregaEstadoDeCuentaComprobanteEgresos(EstadoDeCuenta.Id, f);
				if (f.TipoComprobanteId == 4 || f.TipoComprobanteId == 2)
				{
		...
		private void cmbBancos_SelectedIndexChanged(object sender, EventArgs e)
		{
			try
			{
				...
				{
					tcEstadosCuentaGeneral.Enabled = Convert.ToBoolean(cmbBancos.SelectedIndex);
					CargaCFDIsinRelacionarEgresos(Program.EmpresaSeleccionada, FechaDesdeFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes), FechaHastaFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes));
					CargaEstadoDeCuentasImportadosEgresos(ObtieneCatalogoGenericoFromCmb(cmbBancos).Id, FechaDesdeFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes), FechaHastaFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes));
				}
			}
			catch (Exception ex)
			{
		...
		private void gvXMLs_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			try
			{
				...
				if (e.ColumnIndex == 2)
				{
					...
					fac.Estatus = false;
					gvXMLs.Refresh();
					SeleccionaRelacionEgresos vSelFac = new SeleccionaRelacionEgresos(fac, fac.TipoComprobanteId);
					if (vSelFac.ShowDialog() == DialogResult.OK)
					{
						...
						if (fac.TipoComprobanteId == 1 || fac.TipoComprobanteId == 16)
						{
							...
							if (fac.Estatus && totalDeposito > 0m)
							{
								...
								SeleccionaRelacionEgresos vSelAnt = new SeleccionaRelacionEgresos(fac, true, totalDeposito);
								if (vSelAnt.ShowDialog() == DialogResult.OK)
								{
									fac.NumeroFactura = fac.NumeroFactura + " | " + vSelAnt.TipoMovimientoSeleccionado.Descripcion;
						...
						else if (fac.TipoComprobanteId == 15)
						{
		...
		private void VerificaDepositosRelacionados(List<EntEstadoDeCuenta> ListaFacturas)
		{
			...
			{
				MandaExcepcion("Faltan Retiros por relacionar");
			}
		}
		...
		private void btnImportaXMLs_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					ec.BancoId = bancoId;
					AgregarEstadoDeCuentaEgresos(ec);
				}
		...
		private void btnMarcarNoDepositara_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					f.TipoRelacionId = 2;
					new BusFacturas().ActualizaComprobanteFiscalTipoRelacionEgresos(f.Id, f.TipoRelacionId);
					new BusFacturas().ActualizaComprobanteFiscalDepositadoEgresos(f.Id, true);
					new BusFacturas().AgregaComprobantesFiscalesNoDepositoEgresos(f);
				}
				CargaCFDIsinRelacionarEgresos(Program.EmpresaSeleccionada, FechaDesdeFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes), FechaHastaFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes));
			}
			catch (Exception ex)
		...
		private void btnMarcarDepositaraPosterior_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (MuestraMensajeYesNo("¿Seguro desea marcar como 'PAGARÁ EN MES POSTERIOR' el(los) Comprobante(s) selecccionado(s)?") != DialogResult.Yes)
				{
					return;
				}
				List<EntFactura> lstFacturasSeleccionadas = ObtieneListaFacturasFromGVseleccionados(gvCFDIsSinRelacionar);
				foreach (EntFactura f in lstFacturasSeleccionadas)
				{
					f.TipoRelacionId = 3;
					new BusFacturas().ActualizaComprobanteFiscalTipoRelacionEgresos(f.Id, f.TipoRelacionId);
					new BusFacturas().ActualizaEstatusComprobanteFiscalNoDepositadoEgresos(f.Id, false);
					new BusFacturas().ActualizaComprobanteFiscalDepositadoEgresos(f.Id, false);
				}
				CargaCFDIsinRelacionarEgresos(Program.EmpresaSeleccionada, FechaDesdeFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes), FechaHastaFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes));
			}
			catch (Exception ex)
		...
		private void cmbBancoFiltro_SelectedIndexChanged(object sender, EventArgs e)
		{
			try
			{
				if (BancosCargados)
				{
					CargaEstadoDeCuentasImportadosEgresos(ObtieneCatalogoGenericoFromCmb(cmbBancoFiltro).Id, FechaDesde(rdoPorMesComprobantes.Checked, cmbAñoComprobantes.Text, cmbMesesComprobantes.SelectedIndex + 1, dtpFechaDesde.Value), FechaHasta(rdoPorMesComprobantes.Checked, cmbAñoComprobantes.Text, cmbMesesComprobantes.SelectedIndex + 1, dtpFechaHasta.Value));
				}
			}
			catch (Exception ex)
			{
		...
		private void VerificaCFDIEfectivoSinRelacionRelacionarOFP(List<EntFactura> ListaFacturasSinRelacionar)
		{
			foreach (EntFactura f in ListaFacturasSinRelacionar.Where((EntFactura P) => P.FormaPagoId == 1))
			{
				if (f.TipoRelacionId == 0)
				{
					f.TipoRelacionId = 5;
					new BusFacturas().ActualizaComprobanteFiscalTipoRelacionEgresos(f.Id, f.TipoRelacionId);
					new BusFacturas().ActualizaComprobanteFiscalDepositadoEgresos(f.Id, false);
		...
		private void btnRefrescar_Click(object sender, EventArgs e)
		{
			try
			{
				if (BancosCargados)
				{
					SetWaitCursor();
					CargaCFDIsinRelacionarEgresos(Program.EmpresaSeleccionada, FechaDesdeFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes), FechaHastaFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes));
					VerificaCFDIEfectivoSinRelacionRelacionarOFP(ObtieneListaFacturasFromGV(gvCFDIsSinRelacionar));
					CargaEstadoDeCuentasImportadosEgresos(ObtieneCatalogoGenericoFromCmb(cmbBancos).Id, FechaDesde(rdoPorMesComprobantes.Checked, cmbAñoComprobantes.Text, cmbMesesComprobantes.SelectedIndex + 1, dtpFechaDesde.Value), FechaHasta(rdoPorMesComprobantes.Checked, cmbAñoComprobantes.Text, cmbMesesComprobantes.SelectedIndex + 1, dtpFechaHasta.Value));
				}
			}
			catch (Exception ex)
			{
		...
		private void btnVerificaDepositado_Click(object sender, EventArgs e)
		{
			try
			{
				...
				new BusFacturas().VerificaComprobanteFiscalDepositadoEgresos(fac.FacturaRelacionId);
			}
			catch (Exception ex)
			{
				MuestraExcepcion(ex);
		...
		private void btnAgregaDepositoManual_Click(object sender, EventArgs e)
		{
			try
			{
				AgregaRetiro vDepositos = new AgregaRetiro(FechaDesdeFromComboBoxs(cmbMesesComprobantes, cmbAñoComprobantes));
		...
		private void btnEliminaDeposito_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (MuestraMensajeYesNo("¿Desea Eliminar el Retiro(s) seleccionado(s)?") != DialogResult.Yes)
				{
					return;
				}
				foreach (EntFactura dep in lstdepositosSeleccionados)
				{
					List<EntFactura> cfRelacionados = new BusFacturas().ObtieneEstadoDeCuentaDetallesEgresos(dep.Id);
					new BusFacturas().ActualizaEstatusEstadoDeCuentaEgresos(dep.Id, 0);
					new BusFacturas().ActualizaEstatusEstadoDeCuentaComprobantesEgresos(dep.Id, false);
					foreach (EntFactura f in cfRelacionados)
					{
						if (f.TipoComprobanteId == 4 || f.TipoComprobanteId == 2)
						{
							new BusFacturas().AumentaPagoComprobanteFiscal(f.FacturaRelacionId, -f.Pago);
							new BusFacturas().ActualizaComprobanteFiscalTipoRelacion(f.FacturaRelacionId, 1);
						...
						else
						{
							...
							new BusFacturas().ActualizaComprobanteFiscalTipoRelacionEgresos(f.FacturaRelacionId, 0);
						}
					}
				}
				MuestraMensaje("¡Retiro Eliminado!");
				btnRefrescar.PerformClick();
			}
			catch (Exception ex)
			{
		...
		private void btnMarcarDepositadoMesAnterior_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (MuestraMensajeYesNo("¿Seguro desea marcar como 'PAGADO EN MES ANTERIOR' el(los) Comprobante(s) selecccionado(s)?") != DialogResult.Yes)
				{
					return;
				}
				List<EntFactura> lstFacturasSeleccionadas = ObtieneListaFacturasFromGVseleccionados(gvCFDIsSinRelacionar);
		...
		private void btnRevisarAnticiposDepositosPrevios_Click(object sender, EventArgs e)
		{
			try
			{
				new ReportesEgresos().RevisaAnticipoComprobantesEgresos(new BusFacturas().ObtieneComprobantesFiscalesEgresos(Program.EmpresaSeleccionada, FechaDesde(rdoPorMesComprobantes.Checked, cmbAñoComprobantes.Text, cmbMesesComprobantes.SelectedIndex + 1, dtpFechaDesde.Value), FechaHasta(rdoPorMesComprobantes.Checked, cmbAñoComprobantes.Text, cmbMesesComprobantes.SelectedIndex + 1, dtpFechaHasta.Value), 1));
			}
			catch (Exception ex)
			{
				MuestraExcepcion(ex);
		...
		private void btnOtraFormaPago_Click(object sender, EventArgs e)
		{
			try
			{
				if (gvCFDIsSinRelacionar.Rows.Count == 0)
				...
				if (MuestraMensajeYesNo("¿Seguro desea marcar como 'OTRA FORMA DE PAGO' el(los) Comprobante(s) selecccionado(s)?") != DialogResult.Yes)
				{
					return;
				}
				List<EntFactura> lstFacturasSeleccionadas = ObtieneListaFacturasFromGVseleccionados(gvCFDIsSinRelacionar);
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(LeeXML.Pantallas.EstadoDeCuentaEgresos));
			...
			this.btnOtraFormaPago = new System.Windows.Forms.Button();
			...
			this.tcPedidosGrids.Size = new System.Drawing.Size(1303, 599);
			...
			this.tabPage1.Size = new System.Drawing.Size(1295, 568);
			...
			this.tcEstadosCuentaGeneral.Size = new System.Drawing.Size(1291, 446);
			...
			this.tbImportacion.Size = new System.Drawing.Size(1283, 417);
			...
			this.pnlGeneral.Size = new System.Drawing.Size(1281, 417);
			...
			this.flpTotalesTodos.Location = new System.Drawing.Point(127, 387);
			...
			this.txtRutaCarpeta.Size = new System.Drawing.Size(707, 24);
			...
			this.btnRefrescaCargaArchivo.Location = new System.Drawing.Point(835, 0);
			...
			this.gvXMLs.Size = new System.Drawing.Size(1054, 347);
			...
			this.flowLayoutPanel1.Location = new System.Drawing.Point(1177, 37);
			...
			this.btnAgregaDepositoManual.Text = "Agregar Retiro";
			...
			this.flowLayoutPanel14.Location = new System.Drawing.Point(1178, 226);
			...
			this.tpCFDISinRelacionar.Size = new System.Drawing.Size(1283, 417);
			...
			this.flpPUE.Location = new System.Drawing.Point(159, 313);
			...
			this.gvCFDIsSinRelacionar.Size = new System.Drawing.Size(1185, 288);
			...
			this.flowLayoutPanel3.Controls.Add(this.btnOtraFormaPago);
			this.flowLayoutPanel3.Location = new System.Drawing.Point(1184, 22);
			this.flowLayoutPanel3.Margin = new System.Windows.Forms.Padding(1);
			this.flowLayoutPanel3.Name = "flowLayoutPanel3";
			this.flowLayoutPanel3.Size = new System.Drawing.Size(107, 384);
			...
			this.btnMarcarNoDepositara.Visible = false;
			...
			this.btnMarcarDepositaraPosterior.Image = LeeXML.Properties.Resources.Aceptar;
			...
			this.btnMarcarDepositaraPosterior.Text = "Pagará en Mes Posterior";
			...
			this.btnMarcarDepositadoMesAnterior.Text = "Pagado en Mes Anterior";
			...
			this.btnOtraFormaPago.Anchor = System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Right;
			this.btnOtraFormaPago.BackColor = System.Drawing.Color.White;
			this.btnOtraFormaPago.BackgroundImageLayout = System.Windows.Forms.ImageLayout.Zoom;
			this.btnOtraFormaPago.Cursor = System.Windows.Forms.Cursors.Hand;
			this.btnOtraFormaPago.Font = new System.Drawing.Font("Segoe UI", 7f, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, 0);
			this.btnOtraFormaPago.Image = LeeXML.Properties.Resources.Pago;
			this.btnOtraFormaPago.Location = new System.Drawing.Point(4, 280);
			this.btnOtraFormaPago.Margin = new System.Windows.Forms.Padding(4);
			this.btnOtraFormaPago.Name = "btnOtraFormaPago";
			this.btnOtraFormaPago.Size = new System.Drawing.Size(93, 84);
			this.btnOtraFormaPago.TabIndex = 136;
			this.btnOtraFormaPago.Text = "Otra Forma de Pago";
			this.btnOtraFormaPago.TextAlign = System.Drawing.ContentAlignment.BottomCenter;
			this.btnOtraFormaPago.TextImageRelation = System.Windows.Forms.TextImageRelation.ImageAboveText;
			this.btnOtraFormaPago.UseVisualStyleBackColor = false;
			this.btnOtraFormaPago.Click += new System.EventHandler(btnOtraFormaPago_Click);
			...
			this.tabPage2.Size = new System.Drawing.Size(1283, 417);
			...
			this.gvEstadoDeCuentasImportados.Columns.AddRange(this.Banco, this.dataGridViewTextBoxColumn6, this.dataGridViewTextBoxColumn7, this.dataGridViewTextBoxColumn13, this.gvEstadosCuentaColumPago, this.dataGridViewTextBoxColumn8, this.dataGridViewTextBoxColumn9, this.dataGridViewTextBoxColumn11);
			...
			this.gvEstadoDeCuentasImportados.Size = new System.Drawing.Size(1187, 284);
			...
			this.flowLayoutPanel4.Location = new System.Drawing.Point(159, 318);
			...
			this.flowLayoutPanel10.Location = new System.Drawing.Point(1184, 27);
			...
			this.btnEliminaDeposito.Text = "Eliminar Retiro";
			...
			this.label24.Location = new System.Drawing.Point(494, 16);
			...
			this.cmbEmpresas.Location = new System.Drawing.Point(565, 9);
			...
			this.btnBuscaEmpresa.Location = new System.Drawing.Point(1002, 7);
			...
			this.dataGridViewTextBoxColumn7.HeaderText = "Monto Retiro";
			...
			this.dataGridViewTextBoxColumn9.FillWeight = 350f;
			...
			base.ClientSize = new System.Drawing.Size(1328, 643);
			...
			base.Name = "IngresoEstadoDeCuentaEgresos";
			this.Text = "Importa Estado de Cuenta";
