...
{
	public class Empresas : FormBase
	{
		...
		{
			ListaLicencias = new BusUsuarios().ObtieneLicenciasActivas(CompañiaId);
		...
		private void CargaEmpresasEnPantallas()
		{
		}
		...
		private void btnRegistraDescargaMasiva_Click(object sender, EventArgs e)
		{
			try
			{
				RegistroFIEL vRegCIEC = new RegistroFIEL(EmpresaSeleccionada);
				vRegCIEC.ShowDialog();
				btnRegistraDescargaMasiva.Text = "Cambio CIEC";
				CargaEmpresas();
				btnEmpresaBusqueda.PerformClick();
		...
		private async Task BuscaRFCenRS(string RfcVerifica, string CIEC, bool MostrarMensaje)
		{
			...
			if (RfcVerifica == "PIRP871204DN5")
			{
				...
				foreach (razonesSociales r in ListaRS)
				{
					...
					if (MuestraMensajeYesNo("DESEA ELIMINAR " + r.razon_social + " " + r.rfc) == DialogResult.Yes)
					{
						await EliminarDeLista(r.rfc);
					}
				}
		...
		private async Task SincronizarSolo(string RFC)
		{
			...
			if (response.IsSuccessStatusCode)
			{
				...
				toolStripStatusLabel1.Text = jsonObject["mensaje"].ToString();
				toolStripProgressBar1.Value = 100;
				MuestraMensaje(jsonObject["mensaje"].ToString());
			}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(LeeXML.Pantallas.Empresas));
			...
			this.btnRegistraDescargaMasiva.Size = new System.Drawing.Size(108, 75);
